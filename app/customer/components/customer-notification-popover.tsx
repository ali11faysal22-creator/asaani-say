...
export default function CustomerNotificationPopover() {
    ...
    const loadNotifications = async () => {
      try {
        ...
        setNotifications(sorted.slice(0, 8).map((row) => ({
          id: row.id,
          title: row.body,
          time: (row.created_at || row.createdAt) ? new Date(row.created_at || row.createdAt || '').toLocaleString(undefined, { dateStyle: 'medium', timeStyle: 'short', hour12: true }) : 'Just now',
          unread: !row.is_read,
        })))
      } catch (error) {
  ...
  const handleMarkAllAsRead = async () => {
    const auth = getStoredAuth('customer')
    if (auth?.profile_id) {
      await Promise.all(notifications.filter((item) => item.unread).map((item) => markCustomerNotificationRead(auth.profile_id, item.id).catch(() => undefined)))
    }
