...
export default function NotificationsPage() {
  ...
  const handleBookingAction = async (id: string, status: 'accepted' | 'declined') => {
    ...
    try {
      ...
      if (error instanceof Error && isBookingUnavailableError(error.message)) {
        ...
        setActionError(error instanceof Error ? error.message : 'Unable to decide booking request')
      }
    } finally {
  ...
  const handleTrackingStatus = async (action: Exclude<VendorBookingAction, 'accept' | 'reject'>, reason?: string) => {
    ...
    try {
      ...
      if (error instanceof Error && isBookingUnavailableError(error.message)) {
        const staleId = selectedNotif?.id
        setAssignmentConflict(selectedNotif)
            ...
            <div className="lg:col-span-5">
              {selectedNotif ? (
                <div className="bg-white rounded-2xl border border-slate-200/80 p-6 space-y-5 shadow-xs sticky top-4">
                  {actionError && (
                    <p className="rounded-lg bg-red-50 px-3 py-2 text-xs font-semibold text-red-600">{actionError}</p>
                  )}
                  ...
                  {selectedNotif.booking && ['accepted', 'on_the_way', 'reached', 'in_progress', 'paused', 'work_completed', 'payment_requested'].includes(selectedNotif.booking.status) && (
                    <div className="mt-4 border-t border-slate-100 pt-4">
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        {selectedNotif.booking.status === 'on_the_way' && <button type="button" disabled={actionInProgress} onClick={() => void handleTrackingStatus('reached')} className="rounded-lg bg-orange-500 px-3 py-2 text-[11px] font-bold text-white disabled:opacity-50">Mark reached</button>}
                        {selectedNotif.booking.status === 'reached' && <button type="button" disabled={actionInProgress} onClick={() => void handleTrackingStatus('work_started')} className="rounded-lg bg-orange-500 px-3 py-2 text-[11px] font-bold text-white disabled:opacity-50">Start work</button>}
                        {selectedNotif.booking.status === 'in_progress' && <button type="button" disabled={actionInProgress} onClick={() => void handleTrackingStatus('pause')} className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-[11px] font-bold text-slate-700 disabled:opacity-50">Pause work</button>}
                        {selectedNotif.booking.status === 'paused' && <button type="button" disabled={actionInProgress} onClick={() => void handleTrackingStatus('resume')} className="rounded-lg bg-orange-500 px-3 py-2 text-[11px] font-bold text-white disabled:opacity-50">Resume work</button>}
                        ...
                        {selectedNotif.booking.status === 'work_completed' && <button type="button" disabled={actionInProgress} onClick={() => void handleTrackingStatus('request_payment')} className="rounded-lg bg-emerald-600 px-3 py-2 text-[11px] font-bold text-white disabled:opacity-50">Request payment</button>}
                        {selectedNotif.booking.status === 'payment_requested' && <button type="button" disabled={actionInProgress} onClick={() => void handleTrackingStatus('payment_received')} className="rounded-lg bg-emerald-600 px-3 py-2 text-[11px] font-bold text-white disabled:opacity-50">Payment received</button>}
          ...
          <div className="relative w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
            ...
            <div className="pr-8">
              ...
              <p className="mt-2 text-sm leading-relaxed text-slate-600">
                It was reassigned to another vendor, already decided, or is no longer active. There&apos;s nothing further to do here.
              </p>
            </div>
